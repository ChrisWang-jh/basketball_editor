...
示例：
    python run.py video/wjh_1.mp4 --sam2-repo third_party/sam2
...
from basketeditor.video import read_video_info
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Launch the target annotation, SAM2 tracking, and attack clipping UI.")
    parser.add_argument("video", type=Path, help="Basketball video path")
    parser.add_argument("--sam2-repo", type=Path, default=None, help="Path to the cloned SAM2 repository")
    parser.add_argument("--checkpoint", type=Path, default=REPO_DIR / "checkpoints" / "sam2.1_hiera_base_plus.pt", help="SAM2 checkpoint path")
    parser.add_argument("--model-config", default="configs/sam2.1/sam2.1_hiera_b+.yaml", help="SAM2 model config")
...
def main() -> int:
    ...
    try:
        ...
        info = read_video_info(args.video.expanduser(), cv2)
